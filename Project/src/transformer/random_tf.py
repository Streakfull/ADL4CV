...
from models.transformer_networks.rand_transformer import RandTransformer
...
class RandTransformerModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        assert opt.tf_config is not None
        assert opt.config_path is not None

        # load configs for tf and vq
        tf_conf = OmegaConf.load(opt.tf_config)
        vq_conf = OmegaConf.load(opt.config_path)
        ...
        if self.isTrain:
            ...
            self.optimizer = optim.Adam(
                [p for p in self.tf.parameters() if p.requires_grad == True], lr=opt.lr)
    ...
    def load_vqvae(self, vq_ckpt):
        ...
        print(
            colored('[*] VQVAE: weight successfully load from: %s' % vq_ckpt, 'blue'))
    ...
    def set_input(self, input=None, gen_order=None):
        ...
        x_seq_shuffled = torch.cat(
            [torch.LongTensor(1, bs).fill_(self.sos), x_idx_seq_shuf], dim=0)  # T+1
        # T+1, <sos> should always at start.
        pos_shuffled = torch.cat(
            [self.grid_table[:1], self.grid_table[1:][self.gen_order]], dim=0)
    ...
    def inference(self, data, seq_len=None, gen_order=None, topk=None, prob=None, alpha=1., should_render=False, verbose=False):
        ...
        with torch.no_grad():
            ...
            self.x_recon = self.vqvae.decode_enc_idices(
                self.x_idx, z_spatial_dim=self.grid_size)
    ...
    def shape_comp(self, input, bs=6, topk=30):
        ...
        sdf_partial_cubes = PVQVAEModel.unfold_to_cubes(
            sdf_partial).to(self.opt.device)

        zq_cubes, _, info = self.vqvae.encode(sdf_partial_cubes)
        zq_voxels = PVQVAEModel.fold_to_voxels(
            zq_cubes, batch_size=cur_bs, ncubes_per_dim=8)
        quant = zq_voxels
        _, _, quant_ix = info
        d, h, w = quant.shape[-3:]
        quant_ix = rearrange(
            quant_ix, '(b d h w) -> b d h w', b=cur_bs, d=d, h=h, w=w)
        ...
        input_mesh = get_shape_comp_input_mesh(
            comp_data['sdf'], comp_data['sdf_res'])
    ...
    def backward(self):
        ...
        # exclude the last one as its for <end>
