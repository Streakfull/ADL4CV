...
def sdf_to_mesh(sdf, level=0.02, color=None, render_all=False):
    ...
    mesh = trimesh.Trimesh(p3d_mesh.verts_packed().cpu().numpy(), p3d_mesh.faces_packed().cpu().numpy())
    mesh.fix_normals()
    p3d_mesh = pytorch3d.structures.Meshes([torch.from_numpy(mesh.vertices).float()], [torch.from_numpy(mesh.faces).float()], textures=pytorch3d.renderer.Textures(verts_rgb=verts_rgb)).to(sdf.device)
