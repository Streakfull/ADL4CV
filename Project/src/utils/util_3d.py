...
def init_sdf_to_vox_params(vox_thres=0.033, sxyz=(2.545, 2.545, 2.545), txyz=(0.45, 0.45, 0.45), angles=(-90, 0, 90)):
    ...
    angle = torch.FloatTensor(
        [[np.pi/180 * (anglex), np.pi/180 * (angley), -np.pi/180 * (anglez)]])

    R = pytorch3d.transforms.euler_angles_to_matrix(angle, 'XYZ')
    t = torch.FloatTensor(
        [vrange * tx/64, vrange * ty/64, vrange * tz/64])[None, ..., None]
    S = torch.FloatTensor(
        [[[vrange/sx, 0, 0, 0], [0, vrange/sy, 0, 0], [0, 0, vrange/sz, 0]]])
    Rt = torch.cat([R, t], -1)
...
def get_transform_grids(Rt, S, B, device=None):
    ...
    grid_affine = torch.nn.functional.affine_grid(
        Rt, (B, 1, sdf_size, sdf_size, sdf_size)).to(device)
    grid_scale = torch.nn.functional.affine_grid(
        S, (B, 1, sdf_size, sdf_size, sdf_size)).to(device)
...
def trans_sdf_to_vox(sdf, vox_thres=0.033, grid_to_gt_res=None, grid_affine=None, grid_scale=None, align_corners=True, pad_mode='border'):
    mode = 'bilinear'
    sdf_s = sdf.clone()
    sdf_s = torch.nn.functional.grid_sample(
        sdf_s, grid_affine, mode=mode, align_corners=align_corners, padding_mode=pad_mode)
    sdf_s = torch.nn.functional.grid_sample(
        sdf_s, grid_scale, mode=mode, align_corners=align_corners, padding_mode=pad_mode)
    sdf_s = torch.nn.functional.grid_sample(
        sdf_s, grid_to_gt_res, mode=mode, align_corners=align_corners, padding_mode=pad_mode)
...
def sdf_to_mesh(sdf, level=0.02, color=None, render_all=False):
    ...
    mesh = trimesh.Trimesh(p3d_mesh.verts_packed().cpu(
    ).numpy(), p3d_mesh.faces_packed().cpu().numpy())
    mesh.fix_normals()
    p3d_mesh = pytorch3d.structures.Meshes([torch.from_numpy(mesh.vertices).float()], [torch.from_numpy(
        mesh.faces).float()], textures=pytorch3d.renderer.Textures(verts_rgb=verts_rgb)).to(sdf.device)
